...
def add_conv(in_ch, out_ch, ksize, stride, leaky=True):
    ...
    if leaky:
        ...
        stage.add_module('relu6', nn.ReLU6())
    return stage
...
class LastLevelMaxPool(paddle.nn.Layer):
    def forward(self, x):
        return [F.max_pool2d(x, 1, 2, 0)]


class LastLevelP6P7(paddle.nn.Layer):
    """
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.p6 = nn.Conv2D(in_channels, out_channels, 3, 2, 1, weight_attr=paddle.ParamAttr(initializer=nn.initializer.KaimingUniform()), bias_attr=paddle.ParamAttr(initializer=nn.initializer.Constant(0)))
        self.p7 = nn.Conv2D(out_channels, out_channels, 3, 2, 1, weight_attr=paddle.ParamAttr(initializer=nn.initializer.KaimingUniform()), bias_attr=paddle.ParamAttr(initializer=nn.initializer.Constant(0)))
