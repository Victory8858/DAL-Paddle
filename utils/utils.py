...
def model_info(model, report='summary'):
    ...
    n_g = sum(x.numel() for x in model.parameters() if x.stop_gradient)  # number gradients
    if report is 'full':
...
class Reshape(object):
    ...
    def __call__(self, ims):
        ...
        if self._unsqueeze:
            ims = paddle.unsqueeze(ims, axis=0)
            # ims.unsqueeze(0)
        return ims
...
def get_DOTA_points(label_path, rotate=False):
    ...
    with open(label_path, 'r') as f:
        ...
        for object in lines:
            ...
            x0 = coors[0]; y0 = coors[1]; x1 = coors[2]; y1 = coors[3]; x2 = coors[4]; y2 = coors[5]; x3 = coors[6]; y3 = coors[7]
